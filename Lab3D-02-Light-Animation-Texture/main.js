function init()
{
    ...
    gui.add(cameraZRotation.rotation, 'x', -Math.PI, Math.PI);

    var renderer = new THREE.WebGLRenderer();
    
    renderer.shadowMap.enabled = true;  //chỉnh bóng
...
function getBoxGrid(amount, seperationgMultiplier){
    var group = new THREE.Group();

    for (var i=0; i<amount; i++)   
    {
        var obj = getBox(1, 3, 1);
        obj.position.x = i * seperationgMultiplier;
        ...
        for (var j=1; j<amount; j++)
        {
            ...
            obj.position.x = i * seperationgMultiplier;
            obj.position.y = obj.geometry.parameters.height/2;
            obj.position.z = j * seperationgMultiplier;
            group.add(obj);
        }
    }

    group.position.x = -(seperationgMultiplier * (amount-1))/2;
    group.position.z = -(seperationgMultiplier * (amount-1))/2;
...
function update(renderer, scene, camera, controls, clock){
    ...
    /*
    var plane = scene.getObjectByName('plane-1');   //quay
    plane.rotation.y += 0.001;      
    plane.rotation.z += 0.001;  
    
    scene.traverse(function(child) {        //làm giãn box
        child.scale.x += 0.001;
    })
    ...
    */
    ...
    boxGrid.children.forEach(function(child, index){
        ...
        child.scale.y = (noise.simplex2(x, x) + 1) / 2 + 0.001;    //*5:speed
